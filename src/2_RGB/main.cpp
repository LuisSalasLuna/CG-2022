...
//                       /\
        ...
        // --------------------------------------------------------------------
        /*float vertices[] = {//
            0.00f,  0.50f, 0.0f, // top             |A
            0.11f,  0.15f, 0.0f, // |               |B
            0.48f,  0.15f, 0.0f,  // right_top      |C
            0.18f, -0.06f, 0.0f, // |               |D
            0.29f, -0.41f, 0.0f,  // right_bottom   |E
            0.0f,  -0.19f, 0.0f,  // -              |F
           -0.3f,  -0.41f, 0.0f,  // left_bottom    |G
           -0.18f,  -0.06f, 0.0f,  // |             |H
           -0.48f,  0.15f, 0.0f,  // left_top       |I
           -0.11f,  0.15f, 0.0f, // |               |J
            0.00f,  0.50f, 0.0f, // top
        };*/
    ...
    /*int size = 0;
    if (forma == 0) { glBufferData(GL_ARRAY_BUFFER, sizeof(t), t, GL_STATIC_DRAW); size = 4; }
    if (forma == 1) { glBufferData(GL_ARRAY_BUFFER, sizeof(e), e, GL_STATIC_DRAW); size = 4; }*/
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (forma == 3) {
            ...
            //if (modo == 2) glDrawArrays(GL_TRIANGLES, 385, 720);// Con triangulos Interesante
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_M) == GLFW_PRESS)
    {
        glBindVertexArray(VAO);
        glBindBuffer(GL_ARRAY_BUFFER, VBO);
        glBufferData(GL_ARRAY_BUFFER, (27 + 1128 + 2160)*4, p2, GL_STATIC_DRAW); // arreglar size
        forma = 3;
        //cout<<sizeof(&p2);
    }
...
float* DrawCircle(float x) {
    ...
    /*
    //Triangulos[360*6] = 2160
    for (int i = 0; i < 360; i++) { // Interesante
        arr[count] = x * cos(i * PI / 180);
        arr[count + 1] = x * sin(i * PI / 180);
        arr[count + 2] = 0.0f;
        arr[count + 3] = arr[count + 4] = arr[count + 5] = 0.0f;
    ...
    //Triangulos[360*9] = 3240
    for (int i = 0; i < 360; i++) { // Algo falta
